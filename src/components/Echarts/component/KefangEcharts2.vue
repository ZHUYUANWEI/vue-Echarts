...
<!-- 客房管理——每日订单数金额统计 -->
<div>
  <div id="KefangEcharts2" :style="{width: widthDiv ,height: heightDiv + 'px'}"></div>
</div>
</template>
...
export default {
    ...
    mounted () {
        ...
        window.addEventListener('resize', function () {
            // 浏览器大小改变时canvas重绘
            that.KefangEcharts2.resize()
        })
    },
    methods: {
        drawLine () {
            // 基于准备好的dom，初始化echarts实例
            this.KefangEcharts2 = this.$echarts.init(
                document.getElementById('KefangEcharts2')
            )
            // 绘制图表
            this.KefangEcharts2.setOption({
